...
class TaskTimelinePlugin extends obsidian_1.Plugin {
    ...
    resolveMarkdownPath(value, fallback) {
        const raw = value.trim() || fallback;
        let path = (0, obsidian_1.normalizePath)(raw);
        if (!path.toLowerCase().endsWith(".md")) {
            path += ".md";
        }
        return path;
    }
    scheduleTimelineDecoration() {
    ...
    decorateTimelineViews() {
        ...
        for (const leaf of leaves) {
            ...
            if (!(view instanceof obsidian_1.MarkdownView))
                continue;
            if (!view.file || view.file.path !== donePath)
                continue;
            ...
            const headings = Array.from(preview.querySelectorAll("h2"));
            for (const heading of headings) {
                const dateWrap = (_a = heading.closest(".el-h2")) !== null && _a !== void 0 ? _a : heading.parentElement;
                if (!dateWrap)
                    continue;
    ...
    extractCompletedTaskBlocks(content) {
        ...
        while (index < lines.length) {
            ...
            const block = [line.slice(baseIndent)];
            index += 1;
            while (index < lines.length) {
    ...
    leadingWhitespace(line) {
        var _a, _b;
        return (_b = (_a = line.match(/^\s*/)) === null || _a === void 0 ? void 0 : _a[0].length) !== null && _b !== void 0 ? _b : 0;
    }
    ...
    repairLegacyFrontmatter(content) {
    ...
    insertIntoDoneContent(doneContent, dateHeading, blocks) {
    ...
    async archiveCompletedTasks(forceNotice) {
        if (this.isProcessing)
            return;
        ...
        try {
            ...
            if (todoPath === donePath) {
                new obsidian_1.Notice("Task Timeline: the to-do and completed files must be different.");
                return;
            }
            const todo = this.app.vault.getAbstractFileByPath(todoPath);
            if (!(todo instanceof obsidian_1.TFile)) {
                if (forceNotice) {
                    new obsidian_1.Notice(`Task Timeline: cannot find ${todoPath}`);
                }
                return;
            }
            const todoContent = await this.app.vault.read(todo);
            const { remaining, completedBlocks } = this.extractCompletedTaskBlocks(todoContent);
            if (completedBlocks.length === 0) {
                if (forceNotice) {
                    new obsidian_1.Notice("Task Timeline: no completed tasks found.");
                }
                return;
            }
            ...
            const newDoneContent = this.insertIntoDoneContent(doneContent, today, completedBlocks);
            ...
            if (forceNotice || this.settings.showArchiveNotice) {
                new obsidian_1.Notice(`Task Timeline: archived ${completedBlocks.length} task${completedBlocks.length === 1 ? "" : "s"} to ${today}.`);
            }
            this.scheduleTimelineDecoration();
        }
        catch (error) {
            ...
            new obsidian_1.Notice(`Task Timeline: archive failed. ${message}`);
        }
        finally {
            window.setTimeout(() => {
                this.isProcessing = false;
...
class TaskTimelineSettingTab extends obsidian_1.PluginSettingTab {
...
module.exports = TaskTimelinePlugin;
